...
umap_test = function(n, dist) {
  ...
  dim = DimPlot(
    ...
    xlim(-10, 10) +
    ylim(-10, 10) +
    ggtitle(glue("n.neigh = {as.character(n)}, min.dist = {as.character(dist)}, res = 0.8")) +
    theme(
  ...
  ggsave(
    glue("{umap_folder}neigh{as.character(n)}_mindist{as.character(dist)}_res0.8.pdf"),
    plot = dim,
    device = "pdf",
    width = 8,
    height = 8,
...
umap_algo_test = function(algo) {
  ...
  dim = DimPlot(
    ...
    xlim(-10, 10) +
    ylim(-10, 10) +
    ggtitle(glue("clust algo = {as.character(algo)}, res = 0.8")) +
    theme(
      text = element_text(size = 25),
      plot.title = element_text(size = 20),
      axis.text.x = element_text(size = 25, color = "black"),
